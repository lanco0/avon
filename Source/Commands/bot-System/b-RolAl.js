...
module.exports.run = async (bot, message, args) => {
  ...
  if (message.guild.ownerId !== message.author.id) {
    if (rol.position >= message.member.roles.highest.position)
      return message
        .reply({
          content: `> ${emojiConfig.başarısız} **Başarısız!** Kullanıyıca rolü alamam. Bu kullanıcı senin üstünde.`,
          allowedMentions: { repliedUser: false },
        })
        .catch((err) => {});
  }
  ...
  if (rol.position >= message.guild.me.roles.highest.position)
    return message
      .reply({
        content: `> ${emojiConfig.başarısız} **Başarısız!** Kullanıyıca rolü alamıyorum. Belirttiğiniz Rol benim üstümde.`,
        allowedMentions: { repliedUser: false },
      })
      .catch((err) => {});
  ...
  if (!user.roles.cache.has(rol.id)) {
    message
      .reply({
        content: `> ❌ **Hata!** Belirtilen rol zaten kullanıcıda bulunmuyor.`,
        allowedMentions: { repliedUser: false },
      })
      .catch((err) => {});
  } else {
    user.roles
      ...
      .then((m) => {
        message.reply({
          content: `> ✅ **Başarılı!** Kullanıcıdan ** ** \`${rol.name}\` ** ** adlı rol alındı.`,
          allowedMentions: { repliedUser: false },
        });
      })
      .catch((er) =>
        message
          .reply({
            content: `> ⚠️ **Hata!** Kullanıcıdan rol alırken bir hata oluştu.\n \`\`\`${er}\`\`\``,
            allowedMentions: { repliedUser: false },
          })
          .catch((err) => {})
      );
