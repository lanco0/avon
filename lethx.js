...
const lethx1 = require("discord.js");
const lethx2 = require("discord.js");
const lethx3 = require("discord.js");

const client1 = new lethx1.Client();
const client2 = new lethx2.Client();
const client3 = new lethx3.Client();

const ayarlar = require('./ayarlar.json');
const config = require('./lethx.json');
...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  // guild.roles.cache.forEach(async function(lethx) {
  // if (lethx.permissions.has("ADMINISTRATOR") || lethx.permissions.has("BAN_MEMBERS") || lethx.permissions.has("MANAGE_GUILD") || lethx.permissions.has("KICK_MEMBERS") || lethx.permissions.has("MANAGE_ROLES") || lethx.permissions.has("MANAGE_CHANNELS")) {
  //   lethx.setPermissions(0).catch(err =>{});}});     

  guild.members.ban(entry.executor.id, {reason: "lethx System | İzinsiz kullanıcı yasaklama"}).catch(e => { })	

  let channel = client1.channels.cache.get(ayarlar.defenderlog)
   if (!channel) return console.log('Ban Koruma Logu Yok!');

   const lethx = new Discord.MessageEmbed()
   .setTimestamp()
  ...
  channel.send(`@everyone`, {embed: lethx}).catch(e => { })	
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  // kickhammer.guild.roles.cache.forEach(async function(lethx) {
  // if (lethx.permissions.has("ADMINISTRATOR") || lethx.permissions.has("BAN_MEMBERS") || lethx.permissions.has("MANAGE_GUILD") || lethx.permissions.has("KICK_MEMBERS") || lethx.permissions.has("MANAGE_ROLES") || lethx.permissions.has("MANAGE_CHANNELS")) {
  // lethx.setPermissions(0).catch(err =>{});}});     

  kickhammer.guild.members.ban(yetkili.id, {reason: "lethx System | izinsiz kullanıcı Kickleme!"}).catch(e => { })	
...
const entry = await member.guild
  ...
  if (member.user.bot) {
  // member.guild.roles.cache.forEach(async function(lethx) {
  // if (lethx.permissions.has("ADMINISTRATOR") || lethx.permissions.has("BAN_MEMBERS") || lethx.permissions.has("MANAGE_GUILD") || lethx.permissions.has("KICK_MEMBERS") || lethx.permissions.has("MANAGE_ROLES") || lethx.permissions.has("MANAGE_CHANNELS")) {
  // lethx.setPermissions(0).catch(err =>{});}});

  member.guild.members.ban(entry.executor.id, {reason: "lethx System | İzinsiz Bot Ekleme!"}).catch(e => { })	
  member.guild.members.ban(member.id, {reason: "lethx System | Bot Koruma Sistemi!"}).catch(e => { })	
...
client1.on('guildUpdate', async (oldGuild, newGuild) => {
  ...
  // newGuild.roles.cache.forEach(async function(lethx) {
  // if (lethx.permissions.has("ADMINISTRATOR") || lethx.permissions.has("BAN_MEMBERS") || lethx.permissions.has("MANAGE_GUILD") || lethx.permissions.has("KICK_MEMBERS") || lethx.permissions.has("MANAGE_ROLES") || lethx.permissions.has("MANAGE_CHANNELS")) {
  // lethx.setPermissions(0).catch(err =>{});}});

  newGuild.members.ban(entry.executor.id, {reason: "lethx System | URL Koruma Sistemi!"}).catch(e => { })	
...
client1.on("guildUpdate", async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, { reason: `lethxcim System | Sunucuyu izinsiz güncellemek.` }).catch(e => { })	
  const moment = require('moment');
  moment.locale('tr');
...
client2.on("roleDelete", async role => {
  ...
  // role.guild.roles.cache.forEach(async function(lethx) {
  // if(lethx.permissions.has("ADMINISTRATOR") || lethx.permissions.has("BAN_MEMBERS") || lethx.permissions.has("MANAGE_GUILD") || lethx.permissions.has("KICK_MEMBERS") || lethx.permissions.has("MANAGE_ROLES") || lethx.permissions.has("MANAGE_CHANNELS")) {
  // lethx.setPermissions(0).catch(err =>{});}});  

  role.guild.members.ban(entry.executor.id, { reason: `lethxcim System | İzinsiz rol silme!` }).catch(e => { })	
...
client2.on("roleCreate", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: `lethxcim System | İzinsiz rol oluşturma!` }).catch(e => { })	
  role.delete({ reason: "lethxcim System | Rol Koruma Sistemi" }).catch(e => { })	
...
client2.on("roleUpdate", async (oldRole, newRole) => {
  ...
  newRole.guild.roles.cache.forEach(async function(lethx) {
  if(lethx.permissions.has("ADMINISTRATOR") || lethx.permissions.has("BAN_MEMBERS") || lethx.permissions.has("MANAGE_GUILD") || lethx.permissions.has("KICK_MEMBERS") || lethx.permissions.has("MANAGE_ROLES") || lethx.permissions.has("MANAGE_CHANNELS")) {
  lethx.setPermissions(0).catch(err =>{});}});  

  newRole.guild.members.ban(entry.executor.id, { reason: `lethxcim System | İzinsiz Rol Güncelleme!` }).catch(e => { })	
...
client3.on("channelDelete", async channel => {
  ...
  // channel.guild.roles.cache.forEach(async function(lethx) {
  // if(lethx.permissions.has("ADMINISTRATOR") || lethx.permissions.has("BAN_MEMBERS") || lethx.permissions.has("MANAGE_GUILD") || lethx.permissions.has("KICK_MEMBERS") || lethx.permissions.has("MANAGE_ROLES") || lethx.permissions.has("MANAGE_CHANNELS")) {
  // lethx.setPermissions(0).catch(err =>{});}});

  channel.guild.members.ban(entry.executor.id, { reason: `lethxcim System | İzinsiz Kanal Silme!` }).catch(e => { })	
  await channel.clone({ reason: "lethxcim System | Kanal Korum Sistemi!" }).then(async kanal => {
...
channel2.send(`@everyone`, {embed: lethx}).catch(e => { })
...
client3.on("channelCreate", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `lethxcim System | İzinsiz Kanal Oluşturma!` }).catch(e => { })
  channel.delete({reason: "lethxcim System | Kanal Koruma Sistemi!"}).catch(e => { })
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  // newChannel.guild.cache.roles.cache.forEach(async function(lethx) {
  // if(lethx.permissions.has("ADMINISTRATOR") || lethx.permissions.has("BAN_MEMBERS") || lethx.permissions.has("MANAGE_GUILD") || lethx.permissions.has("KICK_MEMBERS") || lethx.permissions.has("MANAGE_ROLES") || lethx.permissions.has("MANAGE_CHANNELS")) {
  // lethx.setPermissions(0).catch(err =>{});}});

  newChannel.guild.members.ban(member.id, { reason: `lethxcim System | İzinsiz Kanal Güncellemek!` }).catch(e => { })
...
client3.on("webhookUpdate", async (channel) => {
  ...
  await webhooks.map(x => x.delete({reason: "lethxcim System | Webhook Silindi!"})).catch(err => { });
  channel.guild.members.ban(entry.executor.id, {reason: "lethxcim System | İzinsiz Webhook Açmak!"}).catch(err => { });
...
client3.on("emojiDelete", async (emoji, message) => {
  ...
  channel.send(`@everyone`, {embed: lethx}).catch(err => { })
return;
});
